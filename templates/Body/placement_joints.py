...
def placeJointsArm(side="L", settings={}):
    ...
    metacarpus_Pinky = createJoint(f"PlacementJnt_Pinky_Metacarpus_{side}", (sign * 49.461, 101.317, -6.536), (-119.440- offset,sign * 3.341,sign * -59.948), settings)
    ...
    one_Pinky = createJoint(f"PlacementJnt_Pinky_01_{side}", (sign * 52.981, 95.327, -6.968), (-117.673- offset,sign * -10.984,sign * -85.811), settings)
    ...
    two_Pinky = createJoint(f"PlacementJnt_Pinky_02_{side}", (sign * 53.449, 92.635, -6.585), (-118.305- offset,sign * -9.087,sign * -82.188), settings)
    ...
    three_Pinky = createJoint(f"PlacementJnt_Pinky_03_{side}", (sign * 53.711, 91.075, -6.358), (-117.764- offset,sign * -10.735,sign * -85.330), settings)
    ...
    end_Pinky = createJoint(f"PlacementJnt_Pinky_end_{side}", (sign * 53.845, 89.431, -6.045), (-117.764- offset,sign * -10.735,sign * -85.330), settings)

    clavicle = createJoint(f"PlacementJnt_Clavicle_{side}", (sign * 2.214, 133.598, 0.001), (1.337- offset,sign * 15.585,sign *4.963), settings)
    clavicle_end = createJoint(f"PlacementJnt_Clavicle_end_{side}", (sign * 15.215, 134.727, -3.639), (1.337- offset,sign * 15.585,sign *4.963), settings)
    ...
    cmds.parent(end_Pinky, three_Pinky)
    ...
    cmds.parent(three_Pinky, two_Pinky)
    ...
    cmds.parent(two_Pinky, one_Pinky)
    ...
    cmds.parent(one_Pinky, metacarpus_Pinky)
